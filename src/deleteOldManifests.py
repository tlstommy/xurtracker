...
def deleteOldManifests():
    manifestFiles = []
    files = os.listdir("/home/ubuntu")
    for f in range(len(files)):
        if files[f].find(".content") != -1:
            manifestFiles.append(files[f])
    newestFile = manifestFiles[0]
    newestFileTime = os.path.getctime(os.path.join("/home/ubuntu",manifestFiles[0]))

    #find the newest .content file in the dir
    for f in range(len(manifestFiles)): 
        fileCreatedTime = os.path.getctime(os.path.join("/home/ubuntu",manifestFiles[f]))
    ...
    for f in range(len(manifestFiles)): 
        os.remove(os.path.join("/home/ubuntu",manifestFiles[f]))
