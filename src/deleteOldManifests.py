...
def deleteOldManifests():
    manifestFiles = []
    files = os.listdir("/home/ubuntu/XurTracker")
    print(files)
    ...
    print(newestFile)
    newestFileTime = os.path.getctime(os.path.join("/home/ubuntu/XurTracker",manifestFiles[0]))

    #find the newest .content file in the dir
    for f in range(len(manifestFiles)): 
        fileCreatedTime = os.path.getctime(os.path.join("/home/ubuntu/XurTracker",manifestFiles[f]))
    ...
    for f in range(len(manifestFiles)): 
        os.remove(os.path.join("/home/ubuntu/XurTracker",manifestFiles[f]))
